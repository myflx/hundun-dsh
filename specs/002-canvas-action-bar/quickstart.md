...
- [x] E2E-01 打开画布 → 底部操作栏按序出现四按钮（缩小/重置/放大/刷新），右上角无独立缩放工具
- [x] E2E-02 点放大 → 百分比递增 10% 并夹取上限；点缩小同理下限
...
|------|-------------------|----------|----------|
| E2E-01 | ✅ 通过（playwright 真机） | 四按钮顺序 zoom-out→reset→zoom-in→refresh；右上角无独立缩放工具；无自动布局/聚焦 | T001 |
| E2E-02 | ✅ 通过（playwright 真机） | 放大 110% → 缩小 99%（1.1×0.9） | T002 |
| E2E-03 | ✅ 通过（playwright 真机） | 重置回 100% + translate(0,0) | T002 |
...
| E2E-07 | ✅ 通过（playwright 真机 + 静态检查） | 样式仅用 var(--dsw-alias-*)，无硬编码颜色 | T001 |
