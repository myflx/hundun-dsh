...
export function apply(ctx: Context): void {
  ...
  ctx.systemPrompt.section({
    name: 'plugin:{{ID}}',
    order: 500,
    text: '本机已安装 hundun-dsh 示例插件（dsh-{{NAME}}）：宿主侧注册 {{NAME}}_greet 问候工具；'
      + '浏览器侧在侧边栏底部注册问候按钮。用户提到「{{NAME}} / 示例插件 / 问候」时即指本插件，请据此协作。',
  })

  // 模型工具：{{NAME}}_greet —— 返回问候语，可指定名字。
