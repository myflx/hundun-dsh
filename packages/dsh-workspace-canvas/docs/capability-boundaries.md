...
渠道等编排节点全部隶属该工作区，用关联关系（member / link / 扩展边）表达编排意图。
...
| 边界 | 内容 |
| --- | --- |
| 画布节点 | **工作区 = 强制 scope 容器**（基底节点，`WorkspaceView`）；**编排节点不可脱离工作区而存在**（agent 预设 / 任务 / 渠道…由插件经 `ctx.canvas.registerNodeType()` 注册，画布不做硬编码类型） |
| 数据来源 | 基底节点：官方 feed `ctx.workspaces.list`；编排节点实例：**节点插件自己的数据域**（`data.list` 提供者），画布只存 `ref` 引用 |
...
| scope 不变量 | 每个编排节点恰有一条 `member` 边（归属唯一且必需，缺失 → `meta.invalid='no-scope'`）；编排节点坐标为**区域内坐标**；`link`/扩展边默认仅限同工作区内 |
...
- ✅ 右键菜单：重命名（`rename`）、删除注册（`delete`，**级联删除其成员节点与边**，确认提示）、归档会话
- ✅ 作为编排基底：强制挂 `member` 边（每个编排节点恰一条）；区域内可连 `link`/扩展边
...
- ✅ 画布统一提供：区域内放置/拖拽/选中/连线/布局/持久化；**必须归属某工作区**
...
- kind 命名 `scope:type`；`mutate` 校验 kind 已注册、ref 可解析、member 归属完整；
...
| 能力 | 边界 |
| --- | --- |
| 平移/缩放 | ✅ 空白拖拽平移、滚轮缩放（**zoom 限制 0.3×~3×**）、`resetView`、`focusNode`/`focusWorkspace`（聚焦某工作区区域） |
| 拖拽移动 | ✅ 节点自由移动；拖拽中实时显示、drop 才提交；拖到工作区区域吸附 = 建立/迁移 `member` 边；**拖出所有工作区区域 = 无效**（无游离态） |
| 对齐辅助线 | ✅ 拖动时对其它节点中心 x/y 对齐参考 |
| 连线 | ✅ 画布提供**连线协议**（拖线手势、连接点、可连目标过滤/拒绝提示、边文档存储、默认贝塞尔渲染、`onConnect`/`onDisconnect`/`validate` 钩子）；**具体连线规则由插件经 `registerEdgeRule` 实现**（能否连 `accepts`、连几条 `arity`、连上做什么、长什么样 `renderEdge`、是否跨工作区 `crossScope`）；内置 `member`（强制归属）/`link`（同工作区内自由边）为画布基础设施规则 |
...
| 面板互斥 | 遵守 `dsh-panel-activate` 事件协议：与任务看板（taskboard）、SSH 互斥，激活时清对方属性并广播；收到对方激活即关闭 |
| 侧边栏 | 唯一注入点：工作区搜索行右侧的画布按钮（DOM 注入 + 自愈）。**不注入其它区域**；侧边栏槽位（`sidebar.footer.action` 等）不占用，留给其它插件 |
| host 半区 | 只做系统提示词公告（`systemPrompt.section`）；**不注册工具、不注册路由、不碰设置命名空间**（画布无宿主业务） |
...
2. **是否尊重「工作区 = 强制 scope」不变量？** 节点可脱离工作区存在 / 边可任意跨工作区 → 拒绝（除非显式 `crossScope` 且评审通过）；
...
5. **业务连线规则是否应插件化？** 是 → 走 `ctx.canvas.registerEdgeRule()`（accepts/arity/onConnect/renderEdge），画布只提供连线协议，不实现具体连线业务；
